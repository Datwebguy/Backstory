...
class SidecarStore:
    def __init__(self, path: Path):
        ...
        self.path = path
        # FastAPI dispatches each request to a threadpool worker thread, so
        # this connection is used from many threads across its lifetime
        # (never concurrently within one request). check_same_thread=False
        # plus a lock around every access makes that safe.
